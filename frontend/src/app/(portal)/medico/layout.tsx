  ...
  if (u.estado_medico !== "validado") {
    return (
      <div className="flex flex-col gap-6">
        <h1 className="text-[26px] font-bold">Area medica</h1>
        <Alert>
      ...
      <aside className="flex flex-col gap-5">
        <div className="flex flex-col gap-2">
          ...
          <Estado tono={tonoDeValidacion(u.estado_medico)} className="self-start">
            Acreditacion validada
          </Estado>
        </div>
        <p className="text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground">Areas terapeuticas</p>
        {areas.length === 0 ? (
          <p className="text-sm text-muted-foreground">Aun no hay areas publicadas.</p>
        ) : (
          <nav className="flex flex-col gap-1" aria-label="Areas terapeuticas">
            {areas.map((a) => (
        ...
        <p className="rounded-md border bg-card px-4 py-3 text-xs leading-relaxed text-muted-foreground">
          Informacion dirigida exclusivamente a profesionales de la salud.
        </p>
      </aside>
